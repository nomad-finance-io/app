...
export const UPDATE_AMM_CONFIG_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([49, 60, 174, 136, 154, 28, 116, 200]);

export function getUpdateAmmConfigDiscriminatorBytes(): ReadonlyUint8Array { return fixEncoderSize(getBytesEncoder(), 8).encode(UPDATE_AMM_CONFIG_DISCRIMINATOR); }
...
export function getUpdateAmmConfigInstructionDataEncoder(): FixedSizeEncoder<UpdateAmmConfigInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['param', getU8Encoder()], ['value', getU64Encoder()]]), (value) => ({ ...value, discriminator: UPDATE_AMM_CONFIG_DISCRIMINATOR }));
}

export function getUpdateAmmConfigInstructionDataDecoder(): FixedSizeDecoder<UpdateAmmConfigInstructionData> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['param', getU8Decoder()], ['value', getU64Decoder()]]);
}
...
export type ParsedUpdateAmmConfigInstruction<TProgram extends string = typeof NOMAD_AMM_PROGRAM_ADDRESS, TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[]> = { programAddress: Address<TProgram>;
accounts: {
/** The amm config owner or admin */
