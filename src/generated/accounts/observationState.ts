...
import { assertAccountExists, assertAccountsExist, combineCodec, decodeAccount, fetchEncodedAccount, fetchEncodedAccounts, fixDecoderSize, fixEncoderSize, getAddressDecoder, getAddressEncoder, getArrayDecoder, getArrayEncoder, getBooleanDecoder, getBooleanEncoder, getBytesDecoder, getBytesEncoder, getStructDecoder, getStructEncoder, getU16Decoder, getU16Encoder, getU64Decoder, getU64Encoder, transformEncoder, type Account, type Address, type EncodedAccount, type FetchAccountConfig, type FetchAccountsConfig, type FixedSizeCodec, type FixedSizeDecoder, type FixedSizeEncoder, type MaybeAccount, type MaybeEncodedAccount, type ReadonlyUint8Array } from '@solana/kit';
import { getObservationDecoder, getObservationEncoder, type Observation, type ObservationArgs } from '../types';

export const OBSERVATION_STATE_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([122, 174, 197, 53, 129, 9, 165, 132]);

export function getObservationStateDiscriminatorBytes(): ReadonlyUint8Array { return fixEncoderSize(getBytesEncoder(), 8).encode(OBSERVATION_STATE_DISCRIMINATOR); }

export type ObservationState = { discriminator: ReadonlyUint8Array; 
/** Whether the ObservationState is initialized */
initialized: boolean; 
/** the most-recently updated index of the observations array */
observationIndex: number; poolId: Address; 
/** observation array */
...
export function getObservationStateEncoder(): FixedSizeEncoder<ObservationStateArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['initialized', getBooleanEncoder()], ['observationIndex', getU16Encoder()], ['poolId', getAddressEncoder()], ['observations', getArrayEncoder(getObservationEncoder(), { size: 100 })], ['lastUpdateTimestamp', getU64Encoder()], ['padding', getArrayEncoder(getU64Encoder(), { size: 3 })]]), (value) => ({ ...value, discriminator: OBSERVATION_STATE_DISCRIMINATOR }));
}
...
export function getObservationStateDecoder(): FixedSizeDecoder<ObservationState> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['initialized', getBooleanDecoder()], ['observationIndex', getU16Decoder()], ['poolId', getAddressDecoder()], ['observations', getArrayDecoder(getObservationDecoder(), { size: 100 })], ['lastUpdateTimestamp', getU64Decoder()], ['padding', getArrayDecoder(getU64Decoder(), { size: 3 })]]);
}
